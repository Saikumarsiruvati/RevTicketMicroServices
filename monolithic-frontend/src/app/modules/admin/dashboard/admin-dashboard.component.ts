            ...
            <mat-card-content>
              <div class="event-info">
                ...
                <div class="event-details">
                  <h4>{{event.title}}</h4>
                  <p>{{event.genreOrType}} | {{event.language}}</p>
...
export class AdminDashboardComponent implements OnInit {
  ...
  loadShows(): void {
    this.showService.getAllShows().subscribe({
      next: (shows) => {
        this.allShows = shows.reverse();
        this.allShows.forEach(show => {
          if (show.eventId) {
            this.eventService.getEventById(show.eventId.toString()).subscribe({
              next: (event) => {
                show.event = event;
              },
  ...
  getShowDisplayName(show: any): string {
    if (show.event?.title) {
      return `${show.event.title} - ${new Date(show.showTime).toLocaleString()}`;
    }
    const event = this.allEvents.find(e => e.id == show.eventId);
    if (event) {
      return `${event.title} - ${new Date(show.showTime).toLocaleString()}`;
    }
    return `Show ${show.id} - ${new Date(show.showTime).toLocaleString()}`;
  }
}